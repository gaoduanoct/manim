...
class Grouping(Scene):
    def construct(self):
        ...
        circles.arrange(UP, buff=0.1)
        self.add(circles)

        stars = VGroup(*[Star(color=YELLOW, fill_opacity=1).scale(0.5) for _ in range(24)])
        stars.arrange_in_grid(4, 6, buff=0.2)
        self.add(stars)